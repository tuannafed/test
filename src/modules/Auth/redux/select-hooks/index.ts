...
const stepperSelector = createSelector(
  (state: RootState) => state.auth,
  (data: IAuthState) => {
    const { signUpStep, isOpenLocationPicker } = data;
    return {
      signUpStep,
...
export const useSelectSignUp = () => {
  return useSelector<RootState, any>(stepperSelector);
};
